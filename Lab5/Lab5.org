...
We used a spreadsheet for our calculations. To get the average of a column of observations, we used ~=AVERAGE(C2:C11)~. Since we measured $h$ in centimeters, and our equations use meters, we converted $h$ into meters in its own column. To get the work done by friction in a system, we used ~=2*(C13/D12)^2*(B4+A4)+9.81*C13*(B4-A4)~. To get the statistical error, we used ~=sqrt(stdev(F2:F11)/10)~. To get the error of a measurement, we used ~=sqrt(C20^2+C19^2+C18^2)~. To get the errors, we broke down each error equation into its components:
